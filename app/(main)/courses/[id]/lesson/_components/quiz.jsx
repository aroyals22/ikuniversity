'use client';

import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { useState } from 'react';
import { cn } from '@/lib/utils';
import { CheckCircle, Lock } from 'lucide-react';
import QuizModal from './quiz-modal';

const Quiz = ({ courseId, quizSet, quizStatus, modulesComplete }) => {
	const [open, setOpen] = useState(false);

	const quizzes = quizSet.quizIds.map((quiz) => {
		return {
			id: quiz._id.toString(),
			title: quiz.title,
			description: quiz.description,
			options: quiz.options.map((option) => {
				return {
					label: option.text,
					isTrue: option.is_correct,
				};
			}),
		};
	});

	// Each question worth 1 point
	const totalPoints = quizSet.quizIds ? quizSet.quizIds.length : 0;

	// Determine quiz state
	const isLocked = !modulesComplete;
	const isPassed = quizStatus.passed;
	const isTaken = quizStatus.taken;

	// UI States
	let buttonText = 'Take Quiz';
	let buttonVariant = 'outline';
	let buttonDisabled = false;
	let badgeColor = 'bg-success/20 text-primary';

	if (isLocked) {
		buttonText = '🔒 Complete All Modules First';
		buttonDisabled = true;
		buttonVariant = 'outline';
	} else if (isPassed) {
		buttonText = `✅ Passed (${quizStatus.score}%)`;
		buttonDisabled = true;
		buttonVariant = 'outline';
		badgeColor = 'bg-green-500/20 text-green-700';
	} else if (isTaken) {
		buttonText = `Retake Quiz (Last: ${quizStatus.score}%)`;
		buttonVariant = 'outline';
		badgeColor = 'bg-red-500/20 text-red-700';
	}

	return (
		<>
			<div
				className={cn(
					'max-w-[270px] bg-white border border-border rounded-md dark:bg-gray-800 dark:border-gray-700 overflow-hidden',
					(isLocked || isPassed) && 'opacity-75'
				)}
			>
				<div className='flex h-32 items-center justify-center bg-gradient-to-r from-sky-500 to-indigo-500 px-6 text-center'>
					<span className='text-lg font-semibold text-white'>
						{quizSet.title}
					</span>
				</div>
				<div className='p-4'>
					<div className='flex items-center justify-between gap-6 text-sm mb-2 font-medium text-gray-700'>
						<span>Total Points</span>
						<Badge className={cn('hover:bg-success/20', badgeColor)}>
							{totalPoints}
						</Badge>
					</div>

					{isTaken && !isPassed && (
						<div className='mb-2 text-xs text-red-600 font-medium'>
							Last Score: {quizStatus.score}% - Need 70% to pass
						</div>
					)}

					{isPassed && (
						<div className='mb-2 text-xs text-green-600 font-medium flex items-center gap-1'>
							<CheckCircle className='h-3 w-3' />
							Completed with {quizStatus.score}%
						</div>
					)}

					{isLocked && (
						<div className='mb-2 text-xs text-amber-600 font-medium flex items-center gap-1'>
							<Lock className='h-3 w-3' />
							Locked until modules complete
						</div>
					)}

					<p className='mb-4 font-normal text-gray-500 dark:text-gray-400 text-sm'>
						{quizSet.description || quizSet.title}
					</p>

					<Button
						className={cn(
							'flex gap-2 capitalize w-full',
							!buttonDisabled &&
								'border-sky-500 text-sky-500 hover:text-sky-500 hover:bg-sky-500/5',
							buttonDisabled && 'cursor-not-allowed'
						)}
						variant={buttonVariant}
						disabled={buttonDisabled}
						onClick={() => !buttonDisabled && setOpen(true)}
					>
						<svg
							stroke='currentColor'
							fill='currentColor'
							strokeWidth='0'
							viewBox='0 0 24 24'
							className='h-4 w-4'
							height='1em'
							width='1em'
							xmlns='http://www.w3.org/2000/svg'
						>
							<path fill='none' d='M0 0h24v24H0V0z'></path>
							<path d='M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H8V4h12v12zm-6.49-5.84c.41-.73 1.18-1.16 1.63-1.8.48-.68.21-1.94-1.14-1.94-.88 0-1.32.67-1.5 1.23l-1.37-.57C11.51 5.96 12.52 5 13.99 5c1.23 0 2.08.56 2.51 1.26.37.6.58 1.73.01 2.57-.63.93-1.23 1.21-1.56 1.81-.13.24-.18.4-.18 1.18h-1.52c.01-.41-.06-1.08.26-1.66zm-.56 3.79c0-.59.47-1.04 1.05-1.04.59 0 1.04.45 1.04 1.04 0 .58-.44 1.05-1.04 1.05-.58 0-1.05-.47-1.05-1.05z'></path>
						</svg>
						<span>{buttonText}</span>
					</Button>
				</div>
			</div>

			{!isPassed && !isLocked && (
				<QuizModal
					quizzes={quizzes}
					courseId={courseId}
					quizSetId={quizSet._id.toString()}
					open={open}
					setOpen={setOpen}
				/>
			)}
		</>
	);
};

export default Quiz;
